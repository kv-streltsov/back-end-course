		...
		switch (reqElement){
			case 'title':
				if(body.title || body.title === null){
					...
					if(body.title === null && method === 'PUT'){	errorsMessagesObj.errorsMessages.push({
						"message": "undefined",
						"field": "title"})
						break
					}
					...
					if(body.title.length > 40){errorsMessagesObj.errorsMessages.push({
			...
			case 'author':
				if(body.author || body.author === null){
					...
					if(body.author === null && method === 'PUT'){	errorsMessagesObj.errorsMessages.push({
						"message": "undefined",
						"field": "author"})
						break
					}
					...
					if(body.author.length > 20){errorsMessagesObj.errorsMessages.push({
